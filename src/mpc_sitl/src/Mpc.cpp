...
MPC::MPC() :
mass(1.5), mass_l(1.0), length(0.5), I_x(0.0347563), I_y(0.0458929), I_z(0.0977),
...
t_compute_log(0.0), t_compute_sub(0.0), N_adaptive(N)
{
	I(0,0) = I_x;
	I(1,1) = I_y;
	I(2,2) = I_z;
	...
	u_mpc_temp = MVector::Zero();
	u_mpc_temp(0,0) = mass*g;
	...
	path_set_pub = nh.advertise<nav_msgs::Path>("syd/mpc_path", 10, this); //for RVIZ
	path_set_pub2 = nh.advertise<nav_msgs::Path>("syd/mpc_path2", 10, this); //for trajectory_publisher
	velocity_set_pub = nh.advertise<nav_msgs::Path>("syd/mpc_velocity", 10, this); //for trajectory_publisher
	actuator_set_pub = nh.advertise<mavros_msgs::ActuatorControl>("mavros/actuator_control", 10, this);
...
void MPC::start()
{
	...
	while(!stopFlag && ros::ok())
	{
		...
		if(!initialized)
		{
			...
			computeTSpan(t_span, t_now, dt, N_adaptive);
		...
		else
		{
			...
			for(iter_SLQ=0; iter_SLQ<MAX_SLQ; iter_SLQ++)
			{
				...
				s1 = L*(x_nominal.col(N_adaptive)-x_final);;
				S2 = L;
			...
			if(SIMULATION) 
			{
				...
				for(int i=0; i<N_adaptive-1; i++)
				{
					x_nominal.col(i) = x_temp.col(i+1);
					u_nominal.col(i) = u_temp.col(i+1);
				}
				u_nominal.col(N_adaptive-1) = u_nominal.col(N_adaptive-2);
				x_nominal.col(N_adaptive-1) = x_temp.col(N_adaptive);
				...
				if(PLATFORM==MULTIROTOR) 
					x_nominal.col(N_adaptive) = dynamics_multirotor(x_nominal.col(N_adaptive-1), u_nominal.col(N_adaptive-1), dt); 
				else if(PLATFORM==SLUNGLOAD)
					x_nominal.col(N_adaptive) = dynamics_slungload(x_nominal.col(N_adaptive-1), u_nominal.col(N_adaptive-1), dt); 
				cost_old = computeCost(x_nominal, u_nominal, t_span); //TODO t_span & t_now
...
void MPC::current_pose_callback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
	if(!SIMULATION)
	{
		...
		if(PLATFORM==MULTIROTOR)
		{
			...
			if(ADAPTIVE_ENABLE)
			{
				double dist_to_final = sqrt((x_nominal(0,0)-x_final(0))*(x_nominal(0,0)-x_final(0)) + (x_nominal(1,0)-x_final(1))*(x_nominal(1,0)-x_final(1)) + (x_nominal(2,0)-x_final(2))*(x_nominal(2,0)-x_final(2)));
				N_adaptive = round(dist_to_final*(double)ADAPTIVE_HORIZON);
				if(N_adaptive>N) N_adaptive = N;
				if(N_adaptive<ADAPTIVE_MIN) N_adaptive = ADAPTIVE_MIN;
				std::cout << "N_adaptive : " << N_adaptive << std::endl;
			}
			x_nominal.block(0,0,N,1) = dynamics_multirotor(x_nominal.block(0,0,N,1),u_mpc_temp,dt);
		}
		stateSubFlag = true;
	}
}
...
double MPC::computeCost(const StateNominal &x, const InputNominal &u, const TimeNominal &t_span)
{
	...
	temp_L = 0.5*(x.col(N_adaptive)-x_final).transpose()*L*(x.col(N_adaptive)-x_final);
	for(int i=0; i<N_adaptive; i++)
	{
...
void MPC::publishPath()
{
	//int bufferLength = 20;  //should be less than N
	int bufferLength = N_adaptive;  //should be less than N
	std::ostringstream convert;
	convert << bufferLength;
	std::string bufferLength_str = convert.str();

	//Position
	path_set.header.frame_id = "fcu";
	
	std::vector<geometry_msgs::PoseStamped> temp_poses(N_adaptive);
	for(int i=0; i<N_adaptive; i++)
	{
		//if(i==bufferLength)
		//{
		//	std::vector<geometry_msgs::PoseStamped>  temp_poses2(temp_poses.begin(), temp_poses.begin()+bufferLength);
		//	path_set.poses = temp_poses2;
		//	path_set.header.frame_id = bufferLength_str;
		//	path_set_pub2.publish(path_set);
		//	path_set.header.frame_id = "fcu";
		//}
		temp_poses.at(i).pose.position.x = x_nominal(0,i+1);
		temp_poses.at(i).pose.position.y = x_nominal(1,i+1);
		temp_poses.at(i).pose.position.z = x_nominal(2,i+1);
	}
	path_set.poses = temp_poses;
	path_set_pub.publish(path_set);	

	////Velocity
	//velocity_set.header.frame_id = bufferLength_str;
	//std::vector<geometry_msgs::PoseStamped> temp_twists(bufferLength);
	//for(int i=0; i<bufferLength; i++)
	//{
	//	if(PLATFORM==MULTIROTOR)
	//	{
	//		temp_twists.at(i).pose.position.x = x_nominal(6,i+1);
	//		temp_twists.at(i).pose.position.y = x_nominal(7,i+1);
	//		temp_twists.at(i).pose.position.z = x_nominal(8,i+1);
	//	}
	//	else if(PLATFORM==SLUNGLOAD)
	//	{
	//		temp_twists.at(i).pose.position.x = x_nominal(9,i+1);
	//		temp_twists.at(i).pose.position.y = x_nominal(10,i+1);
	//		temp_twists.at(i).pose.position.z = x_nominal(11,i+1);
	//	}
	//}
	//velocity_set.poses = temp_twists;
	//velocity_set_pub.publish(velocity_set);

	//std::cout << "Thrust : " << u_nominal(0,0) << " Roll : " <<u_nominal(1,0) << " Pitch : " << u_nominal(2,0) << " Yaw : " << u_nominal(3,0) << std::endl;
	u_mpc_temp = u_nominal.block(0,0,4,1);
	actuator_set.controls[3] = u_nominal(0,0)/100.0;	
	actuator_set.controls[0] = u_nominal(1,0)/10.0;	
	actuator_set.controls[1] = -u_nominal(2,0)/10.0;	
	actuator_set.controls[2] = -u_nominal(3,0)/10.0;	
	actuator_set_pub.publish(actuator_set);
}
